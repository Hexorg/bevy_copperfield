...
fn make_stick(length: f32, transform: Transform) -> HalfEdgeMesh {
    ...
    let noise_point = [
        noise_point.x as f64,
        noise_point.y as f64,
        noise_point.z as f64,
    ];
    mesh_ops::transform(
        &mut mesh,
        Transform::from_translation(
            0.2 * noise.get(noise_point) as f32
                * Vec3 {
                    x: 1.0,
                    y: 0.0,
                    z: 1.0,
    ...
    while height <= length {
        ...
        face_ops::transform(
            &mut mesh,
            face,
            Transform::from_rotation(Quat::from_rotation_x(0.01 * noise.get(noise_point) as f32))
                .with_translation(
                    0.05 * noise.get(noise_point) as f32
                        * Vec3 {
                            x: 1.0,
                            y: 0.0,
                            z: 1.0,
