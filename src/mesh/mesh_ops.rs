...
use crate::mesh::attributes::{AttributeKind, TraversalQueries};

use super::{
    attributes::SelectionQueries, edge_ops, FaceId, HalfEdgeId, HalfEdgeMesh, StackVec, VertexId,
};
...
pub fn subdivide(mesh: &mut HalfEdgeMesh) {
    ...
    for &vertex in &original_vertices {
        ...
        let (sum, nf) = mesh
            .goto(vertex)
            .adjacent_faces()
            .fold((Vec3::ZERO, 0.0_f32), |acc, i| {
                (acc.0 + face_points[i.face().unwrap()], acc.1 + 1.0)
            });
        let f = sum / nf;
        let (sum, ne) = mesh
            .goto(vertex)
